...
scored <- function(...) {
  ...
  x <- ordered(x)
  if(!.SV4.) {
    oldClass(x) <- c("ordered","factor","Design")  # 17Jul01
  }
    ...
    if(isna) {
      if(is.null(values))	{
        if(n==0) {
          ...
          if(n>0) {
            x <- seq(oldUnclass(lim[1]), #handles chron
                     oldUnclass(lim[2]),length=n)
          } else {
            x <- pretty(oldUnclass(lim[1:2]), n=-n)
          }
...
"%ia%" <- function(x1, x2) {
  ...
  redo <- function(x,nam) {
    ...
    attr(x,"len") <- len
    if(ass==8) {
      attr(x,"nonlinear") <- rep(FALSE, len)
    }
    x
  }
